...
public class CharacterRecord {
    ...
    public static CharacterRecord[] CHARACTERS = charactersList.stream()
            .map(c -> new CharacterRecord(loadHPStats(c.name()), loadAPStats(c.name()), loadDPStats(c.name()), c.name()))
            .toArray(CharacterRecord[]::new);

    public CharacterRecord(int baseHP, int baseAttack, int baseDefense, String name) {
        this.baseHP = baseHP;
    ...
    private static int loadHPStats(String charName) {
        Entity entity = EntityFactory.getInstance().getEntity(EntityFactory.EntityType.valueOf(charName.toUpperCase(Locale.ROOT)));
        return Integer.parseInt(entity.getEntityConfig().getPropertyValue(EntityConfig.EntityProperties.ENTITY_HEALTH_POINTS.toString()));
    }
